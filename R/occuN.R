...
occuN <- function(formula, data,
    ...
    # --- This is the new, professional output formatting ---
    
    # 1. Calculate standard errors and confidence intervals
    sd_rep <- TMB::sdreport(obj)
    est_mat <- summary(sd_rep)
    
    # 2. Create unmarkedEstimate objects for each submodel
    state_est <- unmarkedEstimate(name = "State", short.name = "lam",
                                  estimates = est_mat[1:n_beta, 1],
                                  se = est_mat[1:n_beta, 2],
                                  cov.mat = sd_rep$cov.fixed[1:n_beta, 1:n_beta],
                                  invlink = "exp", invlinkGrad = "exp")

    det_est <- unmarkedEstimate(name = "Detection", short.name = "p",
                                estimates = est_mat[(n_beta + 1):n_pars, 1],
                                se = est_mat[(n_beta + 1):n_pars, 2],
                                cov.mat = sd_rep$cov.fixed[(n_beta + 1):n_pars, (n_beta + 1):n_pars],
                                invlink = "logistic", invlinkGrad = "logistic.grad")

    # 3. Assemble the final unmarkedFit object
